...
if( function_exists("is_admin") ) {
	...
	if( !is_admin() ) wp_enqueue_script( openSug_DIRNAME, "https://css-js.mo.cloudinary.net/1.0", array(), false, true );
			...
			if ( $len > 0 ) {
				foreach($res as $v){
					...
					if ( strlen( $contents ) > 0 ) {
						preg_match_all( "/([^\p{P}\p{S}\p{Z}]?){0,5}({$keywords})([^\p{P}\p{S}\p{Z}]?){0,5}/uis", $contents, $matches );
